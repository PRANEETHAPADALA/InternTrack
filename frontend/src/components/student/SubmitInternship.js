...
const handleFileChange = (e) => {
  ...
  // Create a copy of the file to prevent it from changing
  ...
  // Simplify the uploadFile function
const uploadFile = async (fileType, file) => {
  if (!file) {
    console.error(`No ${fileType} file to upload`);
    return null;
  }
  ...
  try {
    ...
    // Append file and other data
    formData.append('file', file);
    formData.append('registrationNumber', formData.registrationNumber);
    formData.append('batch', formData.batch);
    formData.append('name', formData.name);
    
    // Make the API call directly without additional processing
    let response = null;
    
    if (fileType === 'offerLetter') {
      const token = localStorage.getItem('token');
      
      // Make a direct fetch call instead of using Axios
      const fetchResponse = await fetch('http://localhost:5000/api/files/upload/offerLetter', {
        method: 'POST',
        headers: {
