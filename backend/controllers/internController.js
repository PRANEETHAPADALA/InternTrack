...
// In backend/controllers/internController.js
exports.verifyInternship = async (req, res, next) => {
  try {
    console.log(`Verifying internship ${req.params.id} by user ${req.user.id}`);
    ...
    console.log('Internship found, updating verification fields');
    ...
    console.error('Error in verifyInternship controller:', error);
    res.status(500).json({ 
      success: false, 
      message: error.message 
    });
